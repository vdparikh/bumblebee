...
import (
	...
	"strings"       // For path prefix checking
...
func main() {
	...
	if frontendDir == "" {
		log.Println("Warning: FRONTEND_DIR environment variable not set. Using default './frontend/build'. This might be an issue if not running locally.")
		frontendDir = "./frontend/build" // Fallback for local development outside Docker
	}

	// Serve static assets (JS, CSS, images, etc.) from the frontend build directory
	// These are typically under a /static path or similar in React builds.
	router.StaticFS("/static", http.Dir(filepath.Join(frontendDir, "static")))

	// Serve specific root files like favicon, manifest, etc.
	staticRootFiles := []string{"favicon.ico", "manifest.json", "logo192.png", "logo512.png", "asset-manifest.json"}
	for _, file := range staticRootFiles {
		router.StaticFile(file, filepath.Join(frontendDir, file))
	}

	// For any other GET request not matching API or specific static files, serve index.html
	// This enables client-side routing for the SPA.
	router.NoRoute(func(c *gin.Context) {
		// If it's an API call or an upload path, it should have been handled already.
		...
		// For all other GET requests, serve the main HTML file of the SPA.
